...
int main(){
    ...
    ComparaDataNasc(alunos, temp, n, i);
...
void ComparaDataNasc(Dados *alunos, Dados temp, int n, int i) {
    ...
    for (i = 0; i < n - 1; i++) {
        for (j = 0; j < n - i - 1; j++) {
            if (alunos[j].datanas.ano < alunos[j+1].datanas.ano) {
                temp = alunos[j];
                alunos[j] = alunos[j+1];
                alunos[j+1] = temp;
            } else if (alunos[j].datanas.ano == alunos[j+1].datanas.ano) {
                if (alunos[j].datanas.mes < alunos[j+1].datanas.mes) {
                    temp = alunos[j];
                    alunos[j] = alunos[j+1];
                    alunos[j+1] = temp;
                } else if (alunos[j].datanas.mes == alunos[j+1].datanas.mes) {
                    if (alunos[j].datanas.dia < alunos[j+1].datanas.dia) {
                        temp = alunos[j];
                        alunos[j] = alunos[j+1];
                        alunos[j+1] = temp;
                    }
